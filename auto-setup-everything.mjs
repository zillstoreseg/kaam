...
console.log('\n🚀 AUTOMATIC PLATFORM SETUP STARTING...\n');

// Step 1: Create all tables
console.log('📋 Step 1: Creating platform tables...');
...
try {
  const { error: tablesError } = await supabase.rpc('exec_sql', { sql: createTablesSql }).maybeSingle();

  if (tablesError) {
    console.log('   Note:', tablesError.message);
  } else {
    console.log('   ✅ Tables created successfully\n');
  }
} catch (err) {
  console.log('   Note: Tables may already exist, continuing...\n');
}

// Step 2: Seed data
console.log('📦 Step 2: Seeding plans and features...');
...
if (!plansData || plansData.length === 0) {
  ...
  for (const plan of plans) {
    await supabase.from('plans').upsert(plan, { onConflict: 'id', ignoreDuplicates: true });
  }
...
if (!featuresData || featuresData.length === 0) {
  ...
  for (const feature of features) {
    await supabase.from('features').upsert(feature, { onConflict: 'key', ignoreDuplicates: true });
  }
...
console.log('👤 Step 3: Creating platform owner account...');
    ...
    if (existingUser) {
      ...
      if (existingRole) {
        ...
        const { error: roleError } = await supabase
          .from('platform_roles')
          .insert({ user_id: existingUser.id, role: 'owner' });

        if (roleError) {
          console.error('   ❌ Error assigning owner role:', roleError.message);
        } else {
...
console.log('   3. Look for "Platform Admin" in the sidebar (crown icon)');
console.log('   4. Click it to access /platform-admin\n');
console.log('════════════════════════════════════════════════════════════════\n');
