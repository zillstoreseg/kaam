...
import { Search, MessageCircle, CheckCircle, Filter, X, AlertTriangle, Phone, Calendar, Clock } from 'lucide-react';
...
export default function InactivePlayers() {
  ...
  async function loadInactivePlayers() {
    ...
    try {
      ...
      contactsData?.forEach((record: any) => {
        contactedSet.add(record.student_id);
        if (!lastContactMap[record.student_id]) {
          lastContactMap[record.student_id] = record.contacted_at;
        }
      });
      ...
      students.forEach((student: any) => {
        ...
        if (daysAbsent >= thresholdDays) {
          inactivePlayers.push({
            ...
            was_contacted: contactedSet.has(student.id),
            last_contact_date: lastContactMap[student.id] || null,
  ...
  async function markAsContacted(playerId: string, branchId: string) {
    try {
      ...
      const { error } = await supabase.from('player_contacts').insert({
        ...
        notes: 'Marked as contacted from Inactive Players page',
      });

      if (error) throw error;

      alert('Player marked as contacted!');
      loadInactivePlayers();
    } catch (error) {
  ...
  function openWhatsApp(player: InactivePlayer) {
    ...
    markAsContacted(player.student_id, player.branch_id);
  }
  ...
  const filteredPlayers = players.filter((player) => {
    ...
    const matchesContacted =
      contactedFilter === 'all' ||
      (contactedFilter === 'contacted' && player.was_contacted) ||
      (contactedFilter === 'not_contacted' && !player.was_contacted);
    ...
    <div className="space-y-6">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex gap-2">
                        <button
                          ...
                          disabled={sendingTo === player.student_id}
                          className="flex items-center gap-1 px-3 py-1 bg-green-600 text-white text-sm rounded hover:bg-green-700 transition disabled:opacity-50"
                        >
                        ...
                        {!player.was_contacted && (
                          <button
                            onClick={() => markAsContacted(player.student_id, player.branch_id)}
                            disabled={sendingTo === player.student_id}
                            className="flex items-center gap-1 px-3 py-1 bg-blue-600 text-white text-sm rounded hover:bg-blue-700 transition disabled:opacity-50"
                          >
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
        ...
        <ul className="space-y-1 text-blue-800 text-sm">
          <li>• Click WhatsApp to send a pre-filled reminder message</li>
          <li>• Click Mark Contacted to track follow-up history</li>
          <li>• Use filters to focus on specific groups of players</li>
          <li>• Contact history is preserved for reporting</li>
        </ul>
      </div>
    </div>
  );
