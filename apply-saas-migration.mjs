...
console.log('Applying SaaS migration...');

try {
  // We need to run each statement separately due to Supabase REST API limitations
  // For now, just log the SQL and direct user to run it
  console.log('\n⚠️  Please run the following SQL in the Supabase SQL Editor:');
  console.log('URL: https://viwgdxffvehogkflhkjw.supabase.co/project/viwgdxffvehogkflhkjw/sql/new');
  console.log('\n=== SQL TO RUN ===\n');
  console.log(sql);
  console.log('\n=== END OF SQL ===\n');
  console.log('After running the SQL, create the platform owner account with:');
  console.log('  node create-platform-owner.mjs');
} catch (error) {
